...
def generate_complex_material(source: Image.Image, strength: float = 1.15) -> Image.Image:
    ...
    if (grayscale_max - grayscale_min) <= 2:
        ...
        return Image.merge("RGBA", (roughness, metalness, ao, height))
    ...
    metalness = Image.blend(chroma, specular, alpha=0.35)
    metalness = ImageEnhance.Contrast(metalness).enhance(0.85 + (strength * 0.3))
    metalness = _lift_black_floor(ImageOps.autocontrast(metalness, cutoff=1), floor=3)
    ...
    raw_height = generate_parallax(rgb_source, strength=max(0.85, min(2.0, strength)))
    height = Image.blend(Image.new("L", raw_height.size, color=127), raw_height, alpha=0.8)
...
_SKYRIM_SE_SUFFIX_INFO: dict[str, tuple[str, str, str]] = {
    ...
    "_cm": (
        ...
        "Generated by this tool as RGBA: R=roughness proxy, G=metalness proxy, "
        "B=ambient-occlusion proxy, A=height/detail mask. Intended for modern complex-material "
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=("msn", "cm"),
        default="msn",
        help="Complex material format/suffix: msn -> _msn (normal+spec alpha), cm -> _cm (packed rough/metal/AO/height).",
    )
    parser.add_argument(
        "--normal-strength",
        type=float,
...
if GUI_AVAILABLE:
    class TextureGeneratorGUI:
        def __init__(self) -> None:
            ...
            self._add_tooltip(complex_format, "🏷 Choose 'msn' for RGBA normal+specular, 'cm' for packed rough/metal/AO/height.\nWhen in doubt, use the format your target shader expects.")
