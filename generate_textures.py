...
def write_rmaos_json_sidecar(texture_path: Path) -> Path:
    """Write a lightweight TruePBR sidecar JSON in the PBRNifPatcher folder."""
    json_path = texture_path.parent / "PBRNifPatcher" / f"{texture_path.stem}.json"
    map_name = texture_path.name
    relative_texture_path = (Path("..") / map_name).as_posix()
